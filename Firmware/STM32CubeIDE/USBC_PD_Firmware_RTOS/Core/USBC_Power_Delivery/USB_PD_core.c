...
{
  __disable_irq();
  HAL_GPIO_WritePin(RESET_PD_GPIO_Port,RESET_PD_Pin,GPIO_PIN_SET);
  HAL_Delay(27);  /*time to be dedected by the source almost error recovery time*/
  HAL_GPIO_WritePin(RESET_PD_GPIO_Port,RESET_PD_Pin,GPIO_PIN_RESET);
...
void ALARM_MANAGEMENT(uint8_t Usb_Port)   
{
  ...
  if ( HAL_GPIO_ReadPin(ALERT_N_GPIO_Port,ALERT_N_Pin)== GPIO_PIN_RESET)
  {
...
extern unsigned int Address;
extern unsigned int AddressSize;
extern uint8_t USB_PD_Interupt_PostponedFlag[USBPORT_MAX];
extern uint32_t Previous_VBUS_Current_limitation[USBPORT_MAX];

void USBC_PD_Init(uint8_t Usb_Port)
{
	hi2c[0]= &hi2c1;
	...
	Final_Nego_done[Usb_Port]=0;
	Go_disable_once[Usb_Port] = 0;
	memset((uint32_t *)PDO_FROM_SRC[Usb_Port], 0, 7*sizeof(USB_PD_SRC_PDOTypeDef));
	memset((uint32_t *)PDO_SNK[Usb_Port], 0, 3*sizeof(USB_PD_SNK_PDO_TypeDef));
